<template>
  <div class="page">
    ...
    <mu-appbar style="width: 100%; position:absolute" id="appBar">
      ...
      <!-- 弹出层 -->
      <mu-button flat slot="right" ref="button" @click="open = !open"
        >字体</mu-button
      >
      <mu-popover class="pop" cover :open.sync="open" :trigger="trigger">
        <mu-list>
          <!-- 设置字体大小 -->
          <mu-list-item>
            <mu-list-item-title @click="littleFont" style="font-size:15px;"
              >字</mu-list-item-title
            >
            <mu-list-item-title>｜</mu-list-item-title>
            <mu-list-item-title @click="bigFont" style="font-size:18px;"
              >字</mu-list-item-title
            >
          </mu-list-item>
          <!-- 设置阅读器背景色 -->
    ...
    >
      <mu-list>
        <mu-list-item v-for="(item, index) in sectionList" :key="index">
          <mu-list-item-title @click="jumpToTarget(item)">{{
            item.label
          }}</mu-list-item-title>
        </mu-list-item>
      </mu-list>
...
export default {
  ...
  methods: {
    showEpub() {
      // 获取传递来的书籍路径
      const bookPathList = this.bookPath.split("/");
      const splitBookPath = "/" + bookPathList[2] + "/" + bookPathList[3];
    ...
    hideBar() {
      ...
      const progressDom = document.getElementsByClassName(
        "progress-wrapper"
      )[0];
      // 隐藏/展示appbar
      if (this.isShow) {
    ...
    onProgressChange: function(percentage) {
      ...
      this.location =
        percentage > 0
          ? this.book.locations.cfiFromPercentage(currentPercentage)
          : 0;
    ...
    onProgressInput: function(percentage) {
      ...
      this.location =
        percentage > 0
          ? this.locations.cfiFromPercentage(currentPercentage)
          : 0;
      this.$refs.progress.style.backgroundSize = `${percentage} % 100%`;
      // 保存阅读进度
      this.saveProgress(this.location);
    },
    refreshLocation() {
